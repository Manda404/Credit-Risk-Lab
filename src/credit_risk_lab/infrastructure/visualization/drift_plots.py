...
def plot_drift_summary(report: pd.DataFrame) -> go.Figure:
    ...
    figure = px.bar(
        ...
        hover_data=["type", "ks", "hellinger"],
        title="Train vs external test drift — PSI by feature",
        template="plotly_white",
    )
    figure.add_vline(
        x=0.10, line_dash="dash", line_color="#ffbf00", annotation_text="review 0.10"
    )
    figure.add_vline(
        x=0.25, line_dash="dash", line_color="#d62728", annotation_text="alert 0.25"
    )
    figure.update_layout(
        height=max(500, 28 * len(report)), xaxis_title="Population Stability Index"
    )
    return figure
...
def plot_categorical_distribution(
    ...
    for sample, series in (("train", train[feature]), ("external_test", test[feature])):
        frequency = (
            series.astype("string").fillna("<missing>").value_counts(normalize=True)
        )
        rows.extend(
            {"category": str(k), "frequency": float(v), "sample": sample}
