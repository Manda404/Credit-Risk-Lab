...
def sample_frame():
    return pd.DataFrame({
        "person_age": [30], "person_gender": ["female"], "person_education": ["Bachelor"],
        "person_income": [50000], "person_emp_exp": [8], "person_home_ownership": ["RENT"],
        "loan_amnt": [10000], "loan_intent": ["MEDICAL"], "loan_int_rate": [10.0],
        "loan_percent_income": [0.2], "cb_person_cred_hist_length": [5],
...
def test_feature_engineer_adds_expected_features():
    ...
    assert {"dti", "log_income", "credit_score_band", "has_default_before"}.issubset(transformed.columns)
...
def test_new_domain_features_are_computed_correctly():
    ...
    assert {
        "age_first_credit", "emp_credit_hist_gap", "rate_per_score_point",
        "income_per_experience_year", "risk_flags_count",
